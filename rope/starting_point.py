"""
Reference RoPE (Rotary Position Embedding) implementation with Llama3 scaling.

Compute cos/sin embeddings from precomputed inverse frequencies and position IDs.

get_inputs(axes_and_scalars, device) receives:
    axes_and_scalars = {"batch_size": int, "seq_len": int, "head_dim": int}

Returns dict with:
    position_ids       [batch_size, seq_len]    int64
    inv_freq           [head_dim/2]              float32  (llama3-scaled)
    attention_scaling  scalar                    float32

run(position_ids, inv_freq, attention_scaling) returns:
